...
class testRequestHandler:
    def __init__(self,data:dict):
        ...
        self.data=data
        logger.info(f"{Path(__file__).name}: Request handler called for {self.data}")

        # Configuration Manager for the testReply module of the code
        self.config=testReplyConfigurationManager(data=self.data)
        logger.info(f"{Path(__file__).name}: `testReplyConfigurationManager` called.")
        ...
        logger.info(f"{Path(__file__).name}: returning from database")
    @handle_exceptions
    def returnConfig(self):
        ...
        if self.success:
            ...
            logger.info(f"{Path(__file__).name}: There is no duplicate entry for the data.")

            # If the test is `testByDifficulty`
            if self.data["testType"]=="testByDifficulty":
                logger.info(f"{Path(__file__).name}: The test type is `testByDifficulty`")
                ...
                logger.info(f"{Path(__file__).name}: Test Details are saved in database.")

                return configuration
            
            # If the test is `testByCompanies`
            elif self.data["testType"]=="testByCompanies":
                logger.info(f"{Path(__file__).name}: The test type is `testByCompanies`")
                ...
                logger.info("{Path(__file__).name}: Test Details are saved in database.")

                return configuration
            
            # If the test is `testByGenAI`
            elif self.data["testType"]=="testbyGenAI":
                logger.info(f"{Path(__file__).name}: The test type is `testByGenAI`")
            ...
            logger.info("{Path(__file__).name}: Duplicate entry found.")
            return None
